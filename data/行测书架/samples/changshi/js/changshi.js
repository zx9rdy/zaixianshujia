...
var sampleName = 'changshi',
	samplePath = 'samples/changshi/';

function loadPage(page) {

	var img = $('<img />');
	img.load(function() {
		var container = $('.changshi .p'+page);
		img.css({width: '100%', height: '100%'});
		img.appendTo($('.changshi .p'+page));
		...
		when: {
			...
			change: function(event, scale) {
				...
				if (scale==1) {
					...
					$('.changshi').removeClass('animated').addClass('zoom-in');
			...
			zoomOut: function () {
				...
				setTimeout(function(){
					$('.changshi').addClass('animated').removeClass('zoom-in');
				}, 0);
