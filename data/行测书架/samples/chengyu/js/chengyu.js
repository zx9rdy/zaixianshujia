...
var sampleName = 'chengyu',
	samplePath = 'samples/chengyu/';

function loadPage(page) {

	var img = $('<img />');
	img.load(function() {
		var container = $('.chengyu .p'+page);
		img.css({width: '100%', height: '100%'});
		img.appendTo($('.chengyu .p'+page));
		...
		when: {
			...
			change: function(event, scale) {
				...
				if (scale==1) {
					...
					$('.chengyu').removeClass('animated').addClass('zoom-in');
			...
			zoomOut: function () {
				...
				setTimeout(function(){
					$('.chengyu').addClass('animated').removeClass('zoom-in');
				}, 0);
