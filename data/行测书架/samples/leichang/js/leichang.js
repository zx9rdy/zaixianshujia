...
var sampleName = 'leichang',
	samplePath = 'samples/leichang/';

function loadPage(page) {

	var img = $('<img />');
	img.load(function() {
		var container = $('.leichang .p'+page);
		img.css({width: '100%', height: '100%'});
		img.appendTo($('.leichang .p'+page));
		...
		when: {
			...
			change: function(event, scale) {
				...
				if (scale==1) {
					...
					$('.leichang').removeClass('animated').addClass('zoom-in');
			...
			zoomOut: function () {
				...
				setTimeout(function(){
					$('.leichang').addClass('animated').removeClass('zoom-in');
				}, 0);
