...
var sampleName = 'panduan',
	samplePath = 'samples/panduan/';

function loadPage(page) {

	var img = $('<img />');
	img.load(function() {
		var container = $('.panduan .p'+page);
		img.css({width: '100%', height: '100%'});
		img.appendTo($('.panduan .p'+page));
		...
		when: {
			...
			change: function(event, scale) {
				...
				if (scale==1) {
					...
					$('.panduan').removeClass('animated').addClass('zoom-in');
			...
			zoomOut: function () {
				...
				setTimeout(function(){
					$('.panduan').addClass('animated').removeClass('zoom-in');
				}, 0);
