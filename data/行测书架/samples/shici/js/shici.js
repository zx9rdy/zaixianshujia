...
var sampleName = 'shici',
	samplePath = 'samples/shici/';

function loadPage(page) {

	var img = $('<img />');
	img.load(function() {
		var container = $('.shici .p'+page);
		img.css({width: '100%', height: '100%'});
		img.appendTo($('.shici .p'+page));
		...
		when: {
			...
			change: function(event, scale) {
				...
				if (scale==1) {
					...
					$('.shici').removeClass('animated').addClass('zoom-in');
			...
			zoomOut: function () {
				...
				setTimeout(function(){
					$('.shici').addClass('animated').removeClass('zoom-in');
				}, 0);
