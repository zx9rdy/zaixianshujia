...
var sampleName = 'shuliang',
	samplePath = 'samples/shuliang/';

function loadPage(page) {

	var img = $('<img />');
	img.load(function() {
		var container = $('.shuliang .p'+page);
		img.css({width: '100%', height: '100%'});
		img.appendTo($('.shuliang .p'+page));
		...
		when: {
			...
			change: function(event, scale) {
				...
				if (scale==1) {
					...
					$('.shuliang').removeClass('animated').addClass('zoom-in');
			...
			zoomOut: function () {
				...
				setTimeout(function(){
					$('.shuliang').addClass('animated').removeClass('zoom-in');
				}, 0);
