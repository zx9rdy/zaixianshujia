...
var sampleName = 'yanyu',
	samplePath = 'samples/yanyu/';

function loadPage(page) {

	var img = $('<img />');
	img.load(function() {
		var container = $('.yanyu .p'+page);
		img.css({width: '100%', height: '100%'});
		img.appendTo($('.yanyu .p'+page));
		...
		when: {
			...
			change: function(event, scale) {
				...
				if (scale==1) {
					...
					$('.yanyu').removeClass('animated').addClass('zoom-in');
			...
			zoomOut: function () {
				...
				setTimeout(function(){
					$('.yanyu').addClass('animated').removeClass('zoom-in');
				}, 0);
