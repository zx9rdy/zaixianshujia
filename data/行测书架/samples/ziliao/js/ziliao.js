...
var sampleName = 'ziliao',
	samplePath = 'samples/ziliao/';

function loadPage(page) {

	var img = $('<img />');
	img.load(function() {
		var container = $('.ziliao .p'+page);
		img.css({width: '100%', height: '100%'});
		img.appendTo($('.ziliao .p'+page));
		...
		when: {
			...
			change: function(event, scale) {
				...
				if (scale==1) {
					...
					$('.ziliao').removeClass('animated').addClass('zoom-in');
			...
			zoomOut: function () {
				...
				setTimeout(function(){
					$('.ziliao').addClass('animated').removeClass('zoom-in');
				}, 0);
