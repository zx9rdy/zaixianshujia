$(function() {
    ...
    $('#paging').paging({
        nowPage: 1,
        allPages: Math.ceil(data.count / 4),/*总页码，数字越小，页数越大，0为无穷页*/
        displayPage: 5,/*页码显示数*/
        callBack: function(now) {
            ...
            for (var i = 0; i < currentPages; i++) {
                var num = (now - 1) * 6 + i;
                var create_dl = $('<dl></dl>');
                var _html = '<dt>' + '<a href=' + books[num].alt + ' target="_blank"><img src=' + books[num].image + '></a>' + '</dt>' + '<dd>' + '<a href=' + books[num].alt + ' target="_blank" class="title">' + books[num].title + '</a>' + '<p class="author">' + books[num].author + '</p>' + '<p class="publisher">' + books[num].publisher + '</p>' + '</dd>';
